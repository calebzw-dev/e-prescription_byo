...
df = pd.read_excel('C:/xampp/htdocs/webapp/eps/medibot/dataset.xlsx')
...
def getDiseaseInfo(keywords):
    with DDGS() as ddgs:
        results = ddgs.text(keywords, max_results=1)
        # Assuming you want the first result's body
...
def chat_msg():
    ...
    if request.args["message"]=="undefined":
        ...
        if currentState ==-1:
            response.append("Hi "+user_message+", To predict your disease based on symptoms, we need some information about you. Please provide accordingly.")
...
# Ensure database operations are performed within an application context
